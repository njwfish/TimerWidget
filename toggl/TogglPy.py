...
class Endpoints():
    ...
    TAGS = "https://www.toggl.com/api/v8/tags"
...
class Toggl():
    ...
    def decodeJSON(self, jsonString):
        return json.JSONDecoder().decode(jsonString.decode('ascii'))
    ...
    def setAPIKey(self, APIKey):
        ...
        authHeader = codecs.encode(bytes(authHeader,'ascii'),'base64_codec').rstrip()
        authHeader = "Basic " + authHeader.decode('ascii')
    ...
    def request(self, endpoint, parameters=None):
        ...
        return json.loads(self.requestRaw(endpoint, parameters).decode('ascii'))
    ...
    def postRequest(self, endpoint, parameters=None):
        ...
        if parameters == None:
            ...
            return urllib.request.urlopen(urllib.request.Request(endpoint, data=data.encode('ascii'), headers=self.headers)).read() # make request and read the response

    #----------------------------------
    # Methods for managing Time Entries
    #----------------------------------

    def startTimeEntry(self, description, tags, pid):
    ...
    def getWorkspaceProjects(self, id):
        ...
        :return: Projects objects returned from endpoint
        """
        return self.request(Endpoints.WORKSPACES + '/{0}/projects'.format(id))

    def getWorkspaceTags(self, id):
        """
        :param id: Workspace ID by which to query
        :return: Tags objects returned from endpoint
        """
        return self.request(Endpoints.WORKSPACES + '/{0}/tags'.format(id))
        
    def createTag(self, tag, wid):
        data = {
            "tag": {
            "name": tag,
            "wid":wid,
        ...
        response = self.postRequest(Endpoints.TAGS, parameters=data)
        return self.decodeJSON(response)
